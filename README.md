...
| **🤝 Мультиагентная система** | 4 специализированных агента: Analyst (поиск/объяснение), Coder (генерация), Refactor (паттерны/SOLID), Tester (юнит-тесты). Автомаршрутизация по интенту. |
...
| **✏️ Inline-редактирование** | AI редактирует код прямо в редакторе с предпросмотром diff — принять или отклонить. |
| **📥 Хаб моделей** | Скачивание/переключение между 12+ моделями из UI. Загрузка любой модели с HuggingFace или локального чекпоинта. |
| **🔌 Кастомные модели** | Введите любой HuggingFace repo ID или локальный путь для загрузки совместимой модели. |
...
Запросы автоматически классифицируются и маршрутизируются к специализированным агентам:
- Пользователь: "Где реализована аутентификация?"
→ 🔍 Analyst Agent (ищет в кодовой базе, ссылается на файлы)

- Пользователь: "Создай REST endpoint для регистрации"
→ 💻 Coder Agent (генерирует код, следуя конвенциям проекта)

- Пользователь: "Перепиши этот класс с паттерном Strategy"
→ 🔄 Refactor Agent (применяет паттерны, объясняет изменения)

- Пользователь: "Напиши тесты для этого сервиса"
→ 🧪 Tester Agent (генерирует pytest/jest с edge cases)
...
| **RAG по кодобазе** | ✅ ChromaDB | ✅ | ⚠️ Ограничено | ✅ Sourcegraph | ✅ |
...
POST	/chat	Чат с RAG + мультиагенты
...
Бэкенд: FastAPI, HuggingFace Transformers, bitsandbytes, PEFT, TRL, sentence-transformers, ChromaDB

Расширение: TypeScript, VS Code Extension API, Webview API, WebSocket
